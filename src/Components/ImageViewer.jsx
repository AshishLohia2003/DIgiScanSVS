...
console.log(annotationVisibility)
    ...
    const fetchImage = () => {
        setIsLoading(true);
        fetch(`https://digiscansvs.cloud/api/get_image?slide_dir=${encodeURIComponent(slideDir)}`)
            .then(response => {
                if (!response.ok) {
    ...
    const fetchNotes = () => {
        fetch(`https://digiscansvs.cloud/api/get_notes?slide_dir=${encodeURIComponent(slideDir)}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
    ...
    const fetchInfo = () => {
        fetch(`https://digiscansvs.cloud/api/get_info?slide_dir=${encodeURIComponent(slideDir)}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
